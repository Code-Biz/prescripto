...
const addDoctor = async (req, res) => {
    try {
        ...
        if (password.length < 8) {
            return res.json({ success: false, message: "Password must have 8 characters atleast!" });
        }
        ...
        res.json({ success: true, message: "Doctor Added To Database Successfully!" })
...
const adminDashboard = async (req,res) => {
    try {
        ...
        const dashData = {
            ...
            latest_appointments: appointments.reverse().slice(0,2)
        };
...
export { loginAdmin, addDoctor, allDoctorsAdminFrontend, getAdminAppointments, cancellAppointmentByAdmin, adminDashboard } 